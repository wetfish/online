...
function forumProfile($memID)
{
	...
	setupProfileContext(
		array(
			'avatar_choice', 'fish_equipper', 'hr', 'personal_text',
...
function profileLoadEquippedItems()
{
	global $context, $cur_profile;

	$context['member']['inventory'] = loadInventory($context['id_member']);

	return true;
}

function profileSaveEquippedItems(&$value)
{
	global $context;
	...
	foreach ($_POST as $key => $value)
	{
		...
		{
			$itemId = postItemIdToId($key);

			// was this item selected?
			if($value) 
			{
				if(!array_key_exists($itemId, $inventory))
				{
					// you don't even have this item. nice try
					return "invalid_item_id";
				}

				// if yes, fill the appropriate slot
				$item = $inventory[$itemId];
				$newEquipment[$item['equip_slot']] = $itemId;
			}
		}
	}

	// now that we have each slot mapped to an item, update the inventory
	foreach ($newEquipment as $key => $value)
	{
...
// strips "item_" off of the string
function postItemIdToId($postItemId)
{
	$whatIWant = substr($postItemId, strpos($postItemId, "_") + 1);    
	return $whatIWant;
}
