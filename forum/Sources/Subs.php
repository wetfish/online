...
function updateMemberData($members, $data)
{
	...
	// justin nov 28, 2016 - we use a different DB for the inventory and equipment. ignore it so it doesn't attempt to insert into the members table
	$ignoreList = array(
		'fish_equipper',
	);
	...
	foreach ($data as $var => $val)
	{
		...
		elseif ($var == 'birthdate')
			$type = 'date';
		elseif (in_array($var, $ignoreList))
			continue;
...
function constructPageIndex($base_url, &$start, $max_value, $num_per_page, $flexible_start = false)
{
	...
	else
	{
		...
		if ($start > $num_per_page * ($PageContiguous + 1))
			$pageindex .= '<span style="font-weight: bold;" onclick="' . htmlspecialchars('expandPages(this, ' . JavaScriptEscape(($flexible_start ? $base_url : strtr($base_url, array('%' => '%%')) . ';start=%1$d')) . ', ' . $num_per_page . ', ' . ($start - $num_per_page * $PageContiguous) . ', ' . $num_per_page . ');') . '" onmouseover="this.style.cursor = \'pointer\';"><a>...</a></span>';
		...
		if ($start + $num_per_page * ($PageContiguous + 1) < $tmpMaxPages)
			$pageindex .= '<span style="font-weight: bold;" onclick="expandPages(this, \'' . ($flexible_start ? strtr($base_url, array('\'' => '\\\'')) : strtr($base_url, array('%' => '%%', '\'' => '\\\'')) . ';start=%1$d') . '\', ' . ($start + $num_per_page * ($PageContiguous + 1)) . ', ' . $tmpMaxPages . ', ' . $num_per_page . ');" onmouseover="this.style.cursor=\'pointer\';"><a>...</a></span>';
...
function setupMenuContext()
{
	...
	$context['allow_admin'] = allowedTo(array('admin_forum', 'manage_boards', 'manage_permissions', 'moderate_forum', 'manage_membergroups', 'manage_bans', 'send_mail', 'edit_news', 'manage_attachments', 'manage_smileys', 'manage_items'));
