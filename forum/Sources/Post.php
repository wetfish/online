...
function Post()
{
	...
	// justin Nov 26 2016 - Show message if user is banned from this topic 
	if(isBannedFromTopic($topic))
	{
		$context['post_error']['messages'][] = $txt['banned_from_topic_post_failed'];
		$context['error_type'] = 'serious';
	}
...
function JavaScriptModify()
{
	...
	if (isset($_POST['message']))
	{
		...
		else
		{
			...
			if ($smcFunc['htmltrim'](strip_tags(parse_bbc($_POST['message'], false), '<img><iframe>')) === '')
			{
				$post_errors[] = 'no_message';
				unset($_POST['message']);
			}
